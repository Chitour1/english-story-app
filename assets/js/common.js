// assets/js/common.js
// توجيه الروابط + لافتة تسجيل الدخول الموحّدة + محاولة تجديد التوكن بصمت دائماً
...
(function () {
  ...
  function ensureBannerExists() {
    ...
    bar.querySelector("#signin-drive-btn")?.addEventListener("click", async () => {
      try {
        // إجبار الموافقة التفاعلية أول مرة
        lsSet(HAS_CONSENT_KEY, "0");
        await window.Auth.signInInteractive();
        lsSet(HAS_CONSENT_KEY, "1"); // خزّن علامة الموافقة
        hideBanner();
      } catch (e) {
  ...
  // ======== منطق إظهار/إخفاء اللافتة مع تجديد صامت دائماً ========
  async function refreshBannerState() {
    try {
      // 1) إن كان لدينا توكن صالح بالفعل: أخفِ اللافتة
      if (window.Auth?.hasValidToken()) {
        hideBanner();
        return;
      }

      // 2) جرّب دائماً التجديد الصامت حتى لو لم تُخزَّن علامة الموافقة بعد
      try {
        await window.Auth.ensureAccessToken(); // صامت
        // نجاح التجديد الصامت => أخفِ اللافتة واعتبر أن الموافقة مُنحت
        lsSet(HAS_CONSENT_KEY, "1");
        hideBanner();
        return;
      } catch {
        // فشل الصامت؛ نقرّر حسب العلامة المخزّنة
      }

      // 3) لو لم نملك علامة موافقة، نُظهر اللافتة ليضغط المستخدم مرة واحدة فقط
      ...
      // 4) لدينا علامة موافقة ولكن الصامت فشل (نادر) => أظهر اللافتة كحل أخير
      showBanner();
    } catch {
      showBanner();
    }
  ...
  onReady(async () => {
    // توجيه سريع لكل عنصر يحمل data-nav
